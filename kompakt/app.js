...
try{const saved=JSON.parse(localStorage.getItem("promenada-compact-review-v1")||"null");if(saved&&typeof saved==="object")reviewState={read:saved.read||{},done:saved.done||{},priority:saved.priority||{}}}catch{}
function saveReview(){try{localStorage.setItem("promenada-compact-review-v1",JSON.stringify(reviewState))}catch{}}
...
function dayISO(){return new Intl.DateTimeFormat("sv-SE",{timeZone:"Europe/Warsaw"}).format(new Date(data?.collected_at||Date.now()))}
...
function previewPlan(){
 const p=panel("W planie","Kopia: "+shortDate(dayISO()));
 chosen().forEach(a=>{
  ...
  const time=trip?"Wycieczka · sprawdź godziny":day?lessonSpan(day).span:"Brak dnia w kopii";
...
function render(){
 ...
 $("updated").textContent="Kopia: "+datePL(data.collected_at,{day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"});
 $("health").replaceChildren();
 const bad=Object.values(data.accounts).filter(a=>a.status!=="ok");
 if(bad.length)$("health").append(el("div","notice","W kopii pozostał ostatni poprawny odczyt konta: "+bad.map(a=>a.name).join(", ")+"."));
 $("nav").replaceChildren();
...
function overview(){
 ...
 const aside=el("div","overview-aside");aside.append(previewPlan());
 ...
 if(!fresh.length)inbox.append(empty("Wszystkie treści w tej kopii są przejrzane."));
...
function timetable(){
 ...
 if(!dateKeys.length)p.append(empty("Brak odczytanego planu w tej kopii."));
 ...
 dateKeys.forEach(date=>{
  ...
  accounts.forEach(a=>{
   ...
   b.addEventListener("click",()=>{
    ...
    if(!day?.lessons?.length)details.append(empty("W kopii planu nie ma lekcji na ten dzień."));
